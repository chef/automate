...
func init() {
	...
	policy.MapMethodTo("/chef.automate.api.infra_proxy.InfraProxy/SearchListEnvironment", "infra:infraServers:{server_id}:orgs:{org_id}:environments", "infra:infraServers:get", "GET", "/api/v0/infra/servers/{server_id}/orgs/{org_id}/environments/{name}/search/{index}", func(unexpandedResource string, input interface{}) string {
		if m, ok := input.(*request.SearchListEnvironment); ok {
			return policy.ExpandParameterizedResource(unexpandedResource, func(want string) string {
				switch want {
				case "org_id":
					return m.OrgId
