...
do_prepare_restore() {
    ...
    chef-automate diagnostics run $test_diagnostics_filters ~remove-this-tag-after-merge --lb-url "$test_loadbalancer_url" --skip-cleanup --save-file "/tmp/context2"
}
...
do_test_restore() {
    ...
    if chef-automate diagnostics run $test_diagnostics_filters ~remove-this-tag-after-merge --skip-generate --skip-cleanup --save-file "/tmp/context2" >/tmp/context2runlog 2>&1;
    then
