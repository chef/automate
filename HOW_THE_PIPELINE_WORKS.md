...
## Post-Merge Expeditor Build

After merge, Expeditor will build any changed packages, taking into
account dependency information. These packages are then be uploaded to
the Habitat depot and promoted into the dev channel.
...
After the Expeditor build is complete, Expeditor will trigger the
deploy/dev pipeline. This pipeline runs another set of tests on the
newly promoted set of packages. These tests are a combination of new
...
In addition to the pipelines described above, we have some pipelines
that run nightly. Failures in these pipelines do not block merges or
deploys; however, their failure is still taken seriously and
investigated.

Our nightly pipelines can be found in the expeditor
configuration:
...
To see what these pipelines do, you can look at the job configuration
in:

   - .expeditor/a1-migration.pipeline.yml
   - .expeditor/proxy.pipeline.yml
   - .expeditor/basic-a1.pipeline.yml
