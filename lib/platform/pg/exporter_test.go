...
func TestExport(t *testing.T) {
	...
	stdArgs := []string{"pkg", "exec", pg.PgClient, "pg_dump", "--if-exists", "--verbose", "--clean", "--file", "test_database.sql", "--no-privileges", "--no-owner", connURI}
	...
	tests := []struct {
		...
		{"it runs pg_dump with the right --exclude-table options", "", []string{"table1", "table2"},
			[]string{"pkg", "exec", pg.PgClient, "pg_dump",
		...
		{"it runs pg_dump without --no-privileges and --no-owner if User is set", "testuser", []string{},
			[]string{"pkg", "exec", pg.PgClient, "pg_dump", "--if-exists", "--verbose", "--clean", "--file", "test_database.sql", connURI},
			nil, false, false},
		{"it runs pg_dump with -Fc when UseCustomFormat is set to true", "testuser", []string{},
			[]string{"pkg", "exec", pg.PgClient, "pg_dump", "--if-exists",
...
func TestImport(t *testing.T) {
	...
	t.Run("it runs an import without creating a user or restoring ownership when no User is set", func(t *testing.T) {
		...
		mockExec.Expect("Run", command.ExpectedCommand{
			Cmd: "hab",
			Args: []string{"pkg", "exec", pg.PgClient, "psql", "-f", exportFile,
	...
	t.Run("it returns an error if DropDatabase fails", func(t *testing.T) {
		...
		mockExec.Expect("Run", command.ExpectedCommand{
			Cmd: "hab",
			Args: []string{"pkg", "exec", pg.PgClient, "psql", "-f", exportFile, "-v", "ON_ERROR_STOP=true",
				connURI("test_database")},
			Env: testExpectedEnv,
		}).Return(nil)
