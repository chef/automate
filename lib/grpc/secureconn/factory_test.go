...
func TestSecureConnFactory(t *testing.T) {
	...
	t.Run("Test correctly signed client cert and wrong server name", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Regexp(t, "transport: authentication handshake failed: x509: certificate is valid for root-a-service, root-a-service, not root-foo-service", s.Message())
	})
	...
	t.Run("Test incorrectly signed client cert and correct server name", func(t *testing.T) {
		...
		s, ok := status.FromError(err)
		require.True(t, ok)
		assert.Regexp(t, "transport: authentication handshake failed: x509: certificate signed by unknown authority", s.Message())
	})
}
