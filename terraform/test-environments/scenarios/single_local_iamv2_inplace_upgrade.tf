...
module "single_local_iamv2_inplace_upgrade" {
  source = "https://github.com/chef/es-terraform.git//modules/cd_instance_v2"

  # DNS components ( a2-iamv2-local-inplace-upgrade-{{channel}}.cd.chef.co )
  subdomain        = "a2-iamv2-local-inplace-upgrade"
  subdomain_suffix = "-${var.dns_suffix}"

  # Metadata
  meta_title       = "Single Local (Inplace Upgrade) with IAMv2"
...
module "single_local_iamv2_inplace_upgrade_deploy" {
  source = "../modules/chef_automate_install"

  instance_id   = "${module.single_local_iamv2_inplace_upgrade.instance_id}"
  instance_fqdn = "${module.single_local_iamv2_inplace_upgrade.fqdn}"
  ssh_username  = "${module.single_local_iamv2_inplace_upgrade.ssh_username}"
