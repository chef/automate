...
func (db *DB) GetReindexStatus(requestID int) ([]*ReindexRequest, []*ReindexRequestDetailed, string, error) {
	...
	status := map[string]interface{}{
		...
		"indexes": []map[string]string{
			{"index": "index_1", "stage": "running"},
			{"index": "index_2", "stage": "completed"},
		},
	}
	...
	// Returning nil for the actual DB queries since we're hardcoding the response
	return nil, nil, string(statusJSON), nil
}
