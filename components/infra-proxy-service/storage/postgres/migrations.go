...
// CancelMigration Insert a migration entry to the DB for migration completed
func (p *postgres) CancelMigration(ctx context.Context, migrationId, serverId string, totalSucceeded, totalSkipped, totalFailed int64) (storage.Migration, error) {
	return p.insertMigration(ctx, migrationId, serverId, "", int64(constants.CancelMigration), int64(constants.Completed), totalSucceeded, totalSkipped, totalFailed)
}

// FailedCancelMigration Insert a migration entry to the DB for migration completed
func (p *postgres) FailedCancelMigration(ctx context.Context, migrationId, serverId, message string, totalSucceeded, totalSkipped, totalFailed int64) (storage.Migration, error) {
	return p.insertMigration(ctx, migrationId, serverId, message, int64(constants.CancelMigration), int64(constants.Failed), totalSucceeded, totalSkipped, totalFailed)
}
...
func (p *postgres) GetMigrationStatus(ctx context.Context, migrationId string) (storage.MigrationStatus, error) {
	var m storage.MigrationStatus
	query := `select m.migration_id,t.type,s.status_message, t.id, s.id
			  from migration m 
			  join migration_type t on m.type_id=t.id 
			  join migration_status s on m.status_id=s.id 
