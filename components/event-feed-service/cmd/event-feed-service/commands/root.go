...
func init() {
	...
	// global config
	RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.event-feed-service.toml)")
...
func initConfig() {
	viper.SetConfigName(".event-feed-service") // name of config file (without extension)
