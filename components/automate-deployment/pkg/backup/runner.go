...
func (r *Runner) startRestoreOperations(ctx context.Context) {
	...
	r.info("_progress_ : runner.go : 1 : 663 : startRestoreOperations : Before Unloading services :", desiredServices)
	...
	r.info("_progress_ : runner.go : 1 : 669 : startRestoreOperations : After Unloading services :", desiredServices)
...
func (r *Runner) restoreServices(ctx context.Context, desiredServices []*deployment.Service, restoreCtx Context, cancel func()) error {
	...
	r.info("_progress_: runner.go : 1 : 719 : List of desiredServices :", desiredServices)
	r.info("_progress_ : runner.go : 2 : 720 : restoreCtx.restoreBucket :", restoreCtx.restoreBucket)
	// Restore the services in topological order.
	for _, svc := range desiredServices {
		r.infof("Restoring %s", svc.Name())
		r.info("_progress_ : runner.go : 3 : 720 :svc.Name() :", svc.Name())
		// Install the package
		err := r.target.InstallService(restoreCtx.ctx, svc, channel)
		if err != nil {
			r.info("_progress_ : runner.go : 4 : 728 : r.target.InstallService :", err.Error())
		...
		for _, cmd := range binlinks {
			...
			if err != nil {
				r.info("_progress_ : runner.go : 5 : 738 : r.target.BinlinkPackage :", err.Error())
		...
		if err != nil {
			r.info("_progress_ : runner.go : 6 : 746 : r.configRenderer :", err.Error())
		...
		if err != nil {
			r.info("_progress_ : runner.go : 7 : 753 : r.target.SetUserToml :", err.Error())
		...
		if err := executor.Restore(restoreCtx, metadata); err != nil {
			r.info("_progress_ : runner.go : 9 : 814 : executor.Restore :", err.Error())
		...
		if err != nil {
			r.info("_progress_ : runner.go : 10 : 821 : services.AllBinds.DefaultsForService :", err.Error())
		...
		if err := r.target.LoadService(restoreCtx.ctx, svc, target.BindMode(bindInfo.Mode), target.Binds(bindInfo.Specs)); err != nil {
			r.info("_progress_ : runner.go : 11 : 829 : r.target.LoadService :", err.Error())
...
func (r *Runner) info(args ...interface{}) {
	logrus.WithFields(r.defaultFields()).Info(args...)
}
