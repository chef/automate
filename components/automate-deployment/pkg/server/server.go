...
func usesPlatformScaffolding(service *deployment.Service) bool {
	...
	case "secrets-service", "pg-sidecar-service", "teams-service", "authz-service", "session-service", "automate-dex",
		...
		"automate-cs-bookshelf", "automate-cs-oc-erchef", "automate-cs-oc-bifrost":
		return true
	}
	return false
}
...
func (s *server) Upgrade(ctx context.Context, req *api.UpgradeRequest) (*api.UpgradeResponse, error) {
	...
	if err == manifest.NoSuchManifest {
		return nil, status.Errorf(codes.NotFound, "No manifest for the specified version or channel could be found")
	}
	if err != nil {
		return nil, err
	}
