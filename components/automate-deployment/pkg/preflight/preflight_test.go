...
func TestNetstatCheckNoSkipShared(t *testing.T) {
	portsToCheck := []preflight.PortToCheck{
		{
			Description: "https",
			Port:        443,
			Shared:      true,
	...
	check := preflight.PortCheck(portsToCheck, false)
...
1: 00000000000000000000000000000000:0BB8 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000 00000000   988        0 15314 1 0000000000000000 99 0 0 10 0`)
	...
	t.Run("fails when shared port is used", func(t *testing.T) {
		t.Run("when it's in ipv4", func(t *testing.T) {
			ipv4Txt := strings.TrimSpace(`
				sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
				1: 00000000:01BB 00000000:0000 0A 00000000:00000000 00:00000000 00000000    42        0 9064307 1 0000000000000000 99 0 0 10 0
				2: 7501A8C0:BB08 7501A8C0:2790 01 00000000:00000000 00:00000000 00000000    42        0 10309977 1 0000000000000000 20 3 31 10 -1`)
			ipv6Txt := strings.TrimSpace(`
				sl  local_address                         remote_address                        st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
				0: 00000000000000000000000001000000:0277 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000 00000000     0        0 12089 1 0000000000000000 99 0 0 10 0
			...
			err := check.TestFunc(probe)
			require.NoError(t, err)
			// We don't report success for each port because that's the way it was when this code was bash
			probe.AssertFailure()
		})
		...
		t.Run("when it's ipv6", func(t *testing.T) {
			...
			ipv6Txt := strings.TrimSpace(`
				sl  local_address                         remote_address                        st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
				0: 00000000000000000000000001000000:01BB 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000 00000000     0        0 12089 1 0000000000000000 99 0 0 10 0
				1: 00000000000000000000000000000000:0BB8 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000 00000000   988        0 15314 1 0000000000000000 99 0 0 10 0`)
			probe := NewMockTestProbe(t)
			probe.WithFileContents("/proc/net/tcp", []byte(ipv4Txt))
			probe.WithFileContents("/proc/net/tcp6", []byte(ipv6Txt))
	...
	t.Run("fails when non shared port is used", func(t *testing.T) {
		t.Run("when it's in ipv4", func(t *testing.T) {
			ipv4Txt := strings.TrimSpace(`
				sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
...
func TestNetstatCheckSkipShared(t *testing.T) {
	portsToCheck := []preflight.PortToCheck{
		{
			Description: "https",
			Port:        443,
	...
	check := preflight.PortCheck(portsToCheck, true)
...
1: 00000000000000000000000000000000:0BB8 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000 00000000   988        0 15314 1 0000000000000000 99 0 0 10 0`)
	...
	t.Run("succeeds when shared port is used", func(t *testing.T) {
		t.Run("when it's in ipv4", func(t *testing.T) {
			ipv4Txt := strings.TrimSpace(`
				sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
				1: 00000000:01BB 00000000:0000 0A 00000000:00000000 00:00000000 00000000    42        0 9064307 1 0000000000000000 99 0 0 10 0
