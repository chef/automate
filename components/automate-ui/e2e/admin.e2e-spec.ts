import { browser, $, ExpectedConditions as EC, ElementFinder, promise } from 'protractor';
...
describe('Admin pages', () => {
  ...
  describe('IAM v2', () => {
    describe('API Token list', () => {
      ...
      it('displays tokens table', () => {
        expectCountElements('chef-table chef-thead chef-tr', 1);
        expectCountElements('chef-table chef-tbody chef-tr', 2);
      });

      it('displays the returned token info', () => {
        waitForElement('chef-table chef-tbody chef-tr:nth-child(1) chef-td:first-child')
          .then((name) => {
            expect(name.getText())
              .toBe('This token was generated by the chef-automate CLI tool. ' +
                'It has admin level access on the entire Automate API.');
          });

        waitForElement('chef-table chef-tbody chef-tr:nth-child(1) chef-td:nth-child(2)')
          .then((id) => {
            expect(id.getText()).toBe('admin-token');
          });

        waitForElement('chef-table chef-tbody chef-tr:nth-child(1) chef-td:nth-child(3)')
          .then((date) => {
            expect(date.getText()).not.toBeNull();
          });

        waitForElement('chef-table chef-tbody chef-tr:nth-child(1) chef-td:nth-child(4)')
          .then((status) => {
      ...
      describe('control button', () => {
        let controlButtonP: promise.Promise<ElementFinder>;
        beforeEach(() => {
          controlButtonP = waitForElement(
        ...
        it('on copy token, it sends a notification on success', () => {
          ...
          controlButtonP.then((controlButton) => {
            controlButton.click().then(() => {
              ...
              copyOption.click();
              browser.wait(EC.invisibilityOf(copyOption), 5000, 'Copy option should disappear');

              browser.wait(EC.presenceOf(notification), 5000, 'Copy notification alert should appear');
            });
          });
        });
        ...
        it('on delete token click, it removes the token from the list', () => {
          ...
          controlButtonP.then((controlButton) => {
            ...
            controlButton.click().then(() => {
              ...
              browser.wait(EC.visibilityOf(deleteModal), 5000,
                'Delete confirm modal should render');
              // confirm
              ...
              browser.wait(EC.not(EC.presenceOf(tokenToDelete)), 5000, 'Deleted token should disappear');
            });
          });
        });
      });
    ...
    describe('API Token details', () => {
      ...
      it('displays heading', async () => {
        await browser.wait(EC.textToBePresentInElement($('chef-heading'), token.name), 5000);
      });

      it('displays the id in the header', async () => {
        await browser.wait(
          EC.textToBePresentInElement($('header td:nth-child(1)'), token.id), 5000);
      });

      it('displays the date in the header', async () => {
        const pipe = new DatetimePipe;
        await browser.wait(
          EC.textToBePresentInElement($('header td:nth-child(2)'), pipe.transform(
            token.updated_at,
            'ddd, MMMM D, YYYY [at] H:mm a'
          )), 5000);
      });

      it('displays the status in the header', async () => {
        await browser.wait(
          EC.textToBePresentInElement($('header td:nth-child(3)'), 'Inactive'),
          5000);
      });

      it('displays the ID in the details input', async () => {
        await browser.wait(() => {
          const e = $('main section form chef-form-field div input');
          return e.getAttribute('value').then((a) => a === token.name);
        }, 5000);
      });
    });
  });
  ...
  describe('IAM v1', () => {
      ...
      it('displays the returned token info', () => {
        waitForElement('chef-table chef-tbody chef-tr:nth-child(1) chef-td:first-child')
          .then(name => {
            ...
            expect(name.getText())
              .toBe('This token was generated by the chef-automate CLI tool. It has admin level access on the entire Automate API.');
          });
          ...
          controlButtonP.then((controlButton) => {
            controlButton.click().then(() => {
              ...
              browser.wait(EC.visibilityOf(copyOption), 5000,
                'Copy option should render');
              copyOption.click();
              browser.wait(EC.invisibilityOf(copyOption), 5000,
                'Copy option should disappear');
              browser.wait(EC.presenceOf(notification), 5000,
                'Copy notification alert should appear');
            });
          });
        });
        ...
        it('on delete token click, it removes the token from the list', () => {
          ...
          controlButtonP.then((controlButton) => {
            ...
            controlButton.click().then(() => {
              ...
              // ...however, this should be sufficient to make this non-flakey:
              // if the token is gone from the table, the backend call will have been made
              browser.wait(EC.not(EC.presenceOf(tokenToDelete)), 5000,
                'Deleted token should disappear');
            });
          });
        });
      });
    ...
    describe('API Token details', () => {
      ...
      it('displays heading', async () => {
        await browser.wait(
          EC.textToBePresentInElement($('chef-heading'),
            token.description), 5000);
      });
      ...
      it('displays the ID in the details input', async () => {
        await browser.wait(() => {
          const e = $('main section form chef-form-field div input');
          return e.getAttribute('value').then((a) => a === token.description);
        }, 5000);
      });
    });
  });
  ...
  describe('projects list when the max of 6 projects is reached', () => {
    ...
    it('disables the create project button', () => {
      ...
      browser.wait(
        EC.and(
          EC.visibilityOf(createButton),
          () => createButton.getAttribute('disabled').then(v => v === 'true')),
        5000);
    });
  });
});
