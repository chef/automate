...
Chef provides *Chef-managed* policies, which are integral to the operation of Chef Automate. The policy statements in Chef-managed policies cannot be changed.
...
Any of these resources not explicitly assigned to a project are considered *unassigned* with respect to projects.
