-module(notification_hand_project_slack_webhook).
...
delete_resource(Req, State) ->
    {[EntName, OrgName, ProjName], Req1} = deliv_web_utils:extract_bindings([ent_name, org_name, proj_name], Req),
    case notification_config_db:delete(EntName, OrgName, ProjName) of
        ok ->
            {true, Req1, State};
        {error, _Why} ->
...
to_json(Req, State) ->
    {[EntName, OrgName, ProjName], Req1} = deliv_web_utils:extract_bindings([ent_name, org_name, proj_name], Req),
    case notification_config_db:fetch(EntName, OrgName, ProjName) of
        [] ->
            deliv_web_utils:error_response(404, not_found, Req1, State);
        [Config] ->
            JSON = notification_web_utils:to_json(Config),
            deliv_web_utils:content(JSON, Req1, State);
        {error, _Why} ->
            deliv_web_utils:error_response(500, internal_server_error, Req1, State)
    end.
...
handle_parse({Ejson, Req}, State) ->
    ...
    case notification_config:save(slack_webhook, [EntName, OrgName, ProjName], Ejson) of
