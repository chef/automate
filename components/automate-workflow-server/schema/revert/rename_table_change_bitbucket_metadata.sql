-- Revert delivery:rename_table_change_bitbucket_metadata from pg

BEGIN;

ALTER TABLE scm_changes RENAME TO change_bitbucket_metadata;
...
BEGIN
  ...
  DELETE FROM change_bitbucket_metadata
  WHERE change_id IN (
    SELECT id
    FROM changes
