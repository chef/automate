CREATE OR REPLACE FUNCTION test_invalidate_user()
RETURNS SETOF TEXT
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  user_2_token_2 CONSTANT user_tokens.auth_token%TYPE = 'another_big_chaos_monkey_token';
...
BEGIN
  RETURN QUERY SELECT set_has(
    ...
    format('VALUES (%s, %L), (%s, %L), (%s, %L), (%s, %L)', user_1_id, user_1_token,
                                                            user_2_id, user_2_token,
                                                            user_2_id, user_2_token_2,
                                                            user_3_id, user_3_token),
    'Tokens are present before invalidating');

  PERFORM invalidate('token', 'BigCo', 'BigCo Chaos Monkey');

  RETURN QUERY SELECT set_hasnt(
    'SELECT id, auth_token FROM user_tokens',
    format('VALUES (%s, %L), (%s, %L)', user_2_id, user_2_token, user_2_id, user_2_token_2),
    'BigCo Chaos Monkey''s tokens disappeared!');

  RETURN QUERY SELECT set_has(
    'SELECT id, auth_token FROM user_tokens',
    format('VALUES (%s, %L), (%s, %L)', user_1_id, user_1_token, user_3_id, user_3_token),
    'Everyone else''s tokens are still there!');
  ...
  PERFORM invalidate('password', 'BigCo', 'BigCo Chaos Monkey');

  RETURN QUERY SELECT set_hasnt(
    'SELECT id FROM user_passwords',
    format('VALUES (%s)', user_2_id),
    'BigCo''s passwords disappeared!'
  );

  RETURN QUERY SELECT set_has(
    'SELECT id FROM user_passwords',
    format('VALUES (%s), (%s)', user_1_id, user_3_id),
