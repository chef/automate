...
--
-- Note that we filter out patchsets for changes that are not 'open'.
-- This will be OK, though, becaus we'll be doing JOINs later,
-- and the other changes will fall out there.
latest_patchsets AS (
    ...
    WHERE work.sequence_number = latest
      AND work.status = 'open' -- Don't want anything else
),
