...
BEGIN
  ...
  -- Then the latest stage runs for each pipeline's build and
  -- acceptance stages, only for those which haven't been delivered yet
  SELECT ARRAY(
    SELECT MAX(ssr.id)
      FROM scoped_stage_runs AS ssr
      JOIN changes AS c
     ...
     WHERE ssr.enterprise_id = v_enterprise_id
       AND chs.status = 'open'
       AND ssr.stage IN ('build', 'acceptance')
  GROUP BY ssr.stage, ssr.pipeline_id
  ) INTO v_undelivered_changes_latest_stage_runs;

  -- And finally, the latest stage runs for any change that hasn't
  -- been merged yet
  SELECT ARRAY(
    SELECT MAX(ssr.id)
      FROM scoped_stage_runs AS ssr
