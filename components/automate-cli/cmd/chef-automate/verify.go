...
func (v *verifyCmdFlow) RunVerify(config string) error {
	...
	if v.Config.IsExistingInfra() {
		...
		if err != nil {
			return status.Wrap(err, status.ConfigVerifyError, "Cannot run verify command before provision in AWS deployment. Please run 'chef-automate provision-infra config.toml --airgap-bundle automate.aib' to provision your infra")
		}
