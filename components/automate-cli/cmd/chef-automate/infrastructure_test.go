...
func Test_checkLicenseExpiry(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: errors.New("Your Progress® Chef® Automate™ license expired on 18-09-2024 and you are currently on a limited extension period! To get a new license, please contact the Account Team or email us at chef-account-team@progress.com."),
		},
		{
		...
		},
		{
			...
			licenseResult: &LicenseResult{
				Result: LicenseStatus{
					...
					LicenseId:    "id 1",
					ExpirationDate: ExpirationDate{
						Seconds: time.Now().AddDate(0, 0, 365).Unix(),
					},
				},
			},
		},
