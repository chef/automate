...
### Which OpenSSL Ciphers Are Available in My Version of Automate?

Find out which ciphers are available with an OpenSSL command.
The listed ciphers, depending on the chosen TLS version, can then be used to configure Automate's load balancer service.

For example:

```bash
hab pkg exec core/openssl openssl ciphers 'ALL:!aNULL'
...
To show the ciphers restricted to TLSv1.2, run:

```bash
hab pkg exec core/openssl openssl ciphers -v 'ALL:!aNULL' | grep TLSv1.2
