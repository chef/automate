...
[menu]
  [menu.automate]
    ...
    identifier = "automate/deploy_high_availability/deployment/ha_on_prem_deployment.md On-premise Deployment with Chef Managed Database"
    weight = 200
+++
...
This document explains deploying Chef Automate HA on on-premises machines with the Chef Managed Database.
...
- The workspace path should always be `/hab/a2_deploy_workspace`.
- Chef Automate only supports local Linux users. Chef Automate doesn't support AD managed users in nodes.
...
Ensure you have all the resources for existing infrastructure or cloud infrastructure (AWS/Google Cloud Platform).
...
1. Run the following commands to download the latest Automate CLI and airgapped bundle:
    ...
    To download a specific version bundle, replace `latest.aib` with the Chef Automate version number, such as `4.2.59.aib`.
...
To learn more about generating config, see the [Automate HA Config Generation](/automate/ha_config_gen) page.
...
1. Verify the above config using the `verify` subcommand.
    ...
    To learn more about config verification, check [Config Verify Documentation](/automate/ha_verification_check/).

    Once the verification is completed successfully, proceed with the deployment. In case of failure, fix and verify the issue by re-running the verify command.

## Steps to Deploy

1. The following command will run the deployment. The deploy command will first run the verify command internally. To skip a verification process during the deploy command, use the `--skip-verify` flag.

    ```bash
    chef-automate deploy config.toml --airgap-bundle automate. aib
    ```

   To skip verification during deployment, use the `--skip-verify` flag.
...
- You can use the same IP for the frontend nodes for automate and chefserver.
...
Run the `cleanup` command on your bastion host to uninstall Chef Automate HA instances after unsuccessful deployment.
