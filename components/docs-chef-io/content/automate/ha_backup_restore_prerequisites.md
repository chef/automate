...
Once the shared filesystem is mounted to the mount point (`/mnt/automate_backups`), Follow 'Configuring Automate to register the snapshot locations with OpenSearch' section to configure Automate.
...
#### Configuring Automate to register the snapshot locations with OpenSearch
