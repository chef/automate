...
This page explains the prerequisites of the backup. If you choose the AWS Deployment procedure to deploy Automate-HA, you can choose either the `efs` or `s3` backup option. In the config it will be: **`backup_config = "efs"` OR `backup_config = "s3"` in `config.toml`, the below steps are not required.** The below steps are taken care of by the deployment. If you have kept the `backup_config` blank, perform the below steps.
