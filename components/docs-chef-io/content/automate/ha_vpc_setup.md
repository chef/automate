...
A /32 is a single specific address. In this case, 10.10.101.5, not wild carding anything. This is only one IP address that is used to authorize in a security group traffic. Likewise, if you want to authorize traffic from the entire internet, wild card everything. So 000/0 becomes the final notation to the front‑end web servers or other elements.
