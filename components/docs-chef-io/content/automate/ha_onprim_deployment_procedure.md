...
- To provide multiline certificates use triple quotes like `""" multiline certificate contents"""`.
- Rebooting or restarting individual nodes outside of a designated maintenance window especially during periods of high traffic should be avoided.
...
- Transitioning to a 11 node deployment requires decommissioning the existing 5 node cluster entirely. The new 11 node architecture must be provisioned from scratch.
{{< /note >}}
