...
- `Deployment` is a config type, we may have a different type of configs in the future.

## SSH User and Group

- `ssh user name` user name to ssh to cluster instances.
- `ssh group name` group name which is associated with ssh user.
- `ssh port no` port to do ssh, default is 22, in case you have different port then provide the ssh port number.
...
- `Automate Admin Password` Admin password to log in to automate dashboard.
...
- `Private key for Automate` In case to have custom certificate for Automate node provide your private for Automate, If you have custom certificates for each Automate node then provide a different private key for each of Automate nodes.
- `Public key for Automate` In case to have custom certificate for Automate node provide your public for Automate, If you have custom certificates for each Automate node then provide a different public key for each of Automate nodes.

## Private/Public Key For Chef Server

- `Private key for Chef Server` In case to have custom certificate for Chef Server node provide your private for Chef Server, If you have custom certificates for each Chef Server node then provide a different private key for each of Chef Server nodes.
- `Public key for Chef Server` In case to have custom certificate for Chef Server node provide your public for Chef Server, If you have custom certificates for each Chef Server node then provide a different public key for each of Chef Server nodes.
...
- `Private key for Open Search` In case to have custom certificate for Open Search node provide your private for Open Search, If you have custom certificates for each Open Search node then provide a different private key for each of Open Search nodes.
- `Public key for Open Search` In case to have custom certificate for Open Search node provide your public for Open Search, If you have custom certificates for each Open Search node then provide a different public key for each of Open Search nodes.
...
- `Private key for Postgresql` In case to have custom certificate for Postgresql node provide your private for Postgresql, If you have custom certificates for each Postgresql node then provide a different private key for each of Postgresql nodes.
- `Public key for Postgresql` In case to have custom certificate for Postgresql node provide your public for Postgresql, If you have custom certificates for each Postgresql node then provide a different public key for each of Postgresql nodes.
...
- Details required for AWS deployment:
  ...
  - `AMI Id` AWS AMI ID for a specific region to create cluster of particular AMI.
...
In the Case of AWS managed or Customer managed databases below fields will be required.
...
- `Opensearch root-ca` SSL root certificates to connect with opensearch, In Case of AWS managed databases we have an option to use default aws certificates - - If using default certificates then no need to provide root certificates.

### PostgreSQL

- `PostgreSQL URL and port` Postgresql url along with port Example: `postgresql.example.com:5432`.
- `PostgreSQL super username` master username to log in to postgresql.
- `PostgreSQL super user password` master password to log in to postgresql.
...
- `Aws OpenSearch snapshot user accesskey` snapshot user access key is required to take a backup from aws OpenSearch
- `Aws OpenSearch snapshot secret key` snapshot user access key is required to take a backup from aws OpenSearch. Refer to the [Enabling OpenSearch Backup Restore](/automate/managed_services/#enabling-opensearch-backup-restore) section, to create them and get their values.

## Backup

- If backup is to be configured at the time of deployment, then the user needs to provide value to the following fields/prompts:
  ...
  - `Google Service Account File` In case of Google Cloud Storage provide the credential file path Example: `/path/to/file/test.json`.
