...
Chef Supermarket is the site for community cookbooks. It provides a searchable cookbook repository and a friendly web UI. In this page we will discuss the integration of Supermarket with Automate HA set up using On-Premises along with AWS Managed Services, OpenSearch and PostgreSQL.

Before we integrate the supermarket, let's see how to register a new oauth application with OC-ID under Automate.

## Register a New Oauth Application with OC-ID

When you install Chef Automate, it bundles the OC-ID component as a Oauth provider. The Oauth provider uses the Chef-Server credentials for logging in to some other application. For example, Supermarket. Follow the steps given below to register the applications to use OC-ID as a medium to log-in to the respective applications. Once you are done with the registration, you will be authorized to use the application if the Che-Server login credentials are correct.
...
1. Create a file to list down the details of the application you want to register with OC-ID. In the file, mention the **name** and the **redirect_uri** for the file. The content of the created file looks like as shown below:
    ...
    To add more than one applications to the OC-ID, keep adding the above code in the file. For example:
...
1. Now, patch the above file by running the below command from your habitat studio `/src` directory or from your bastion system:
...
1. Verify whether the new configuration has been applied or not by running the following command:
    ...
    The output of the above command will contain the values from the file you had patched by running the patch command.

1. The above command will patch the new configuration and will restart OC-ID service. Under the hood it does a few things as follows:

    * Once you patch the `.toml` file it will update the values in running automate configuration as mentioned in the `.toml` file.

    * It will restart the **automate-cs-ocid** service which will invoke the run hook of that component.

    * The run hook has a default instruction to register a new application if mentioned in the automate configuration.

    * As part of the registration process it basically creates a record for the application and generates a pair of (**uid+secret**) in the OC-ID database.
...
1. Now, configure your application with the generated `uid` and `secret` to be authorized to use **OC-ID** as a oauth provider of chef-server and use credential of chef-server to login to the app.
