...
func (p *pg) GetUserPolicies(ctx context.Context, memberType, username, connectorId string) ([]string, error) {
	...
	WHERE iam_members.name=$1;`, memberType+":"+connectorId+":"+username)
	if err != nil {
		return nil, p.processError(err)
	}
