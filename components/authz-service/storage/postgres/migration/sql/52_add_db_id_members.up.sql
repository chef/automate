...
ALTER TABLE iam_policy_members DROP CONSTRAINT iam_policy_members_member_id_fkey;
ALTER TABLE iam_members DROP CONSTRAINT iam_members_pkey;
ALTER TABLE iam_members ADD COLUMN db_id SERIAL PRIMARY KEY UNIQUE;
ALTER TABLE iam_policy_members RENAME COLUMN member_id TO member_temp_id;
ALTER TABLE iam_policy_members ADD COLUMN member_id INTEGER REFERENCES iam_members (db_id) ON DELETE CASCADE;
UPDATE iam_policy_members ipm SET member_id = (SELECT db_id FROM iam_members WHERE id = ipm.member_temp_id);
ALTER TABLE iam_policy_members DROP COLUMN member_temp_id;
ALTER TABLE iam_members ADD UNIQUE (id, name); 
ALTER TABLE iam_policy_members ADD UNIQUE (member_id, policy_id); 
  ...
  WITH temp AS (
    SELECT
      ...
      ( SELECT array_agg(mem) FILTER (WHERE mem.id IS NOT NULL)
        ...
        LEFT OUTER JOIN iam_members AS mem ON pol_mems.member_id = mem.db_id
