...
CREATE OR REPLACE FUNCTION
  ...
  SELECT json_build_object('id', t.id, 'name',
      t.name, 'type', t.type, 'actions', t.actions,
      'projects', COALESCE(t.projects, '{}') ) AS role FROM t
      -- if empty _projects_filter is passed, return row
    WHERE array_length(_projects_filter, 1) IS NULL
