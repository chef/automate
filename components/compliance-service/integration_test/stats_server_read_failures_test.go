...
func TestReadFailures(t *testing.T) {
	...
	for _, test := range successCases {
		t.Run(test.description, func(t *testing.T) {
			...
			assert.Equal(t, test.expectedPassedCnt, response.Environments[0].Failures, "Environment Failures count")
			//
			//response, err = statsServer.ReadFailures(ctx, &octoberTwentyFifthWithAllFailureTypesQuery)
			//
			//assert.NoError(t, err)
			//require.NotNil(t, response)
			//
