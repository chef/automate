...
BODY = '{
  "size": 10,
  "_source": [ "title", "version" ],
  "query": {
    "bool": {
      "must": [
        {
          "type": {
            "value": "inspec_profile"
          }
        },
        {
          "match": {
            "title.engram": "TEXT_HERE"
          }
        }
      ]
    }
...
request = Net::HTTP::Post.new("#{path}/#{es_index}/_search?filter_path=took,hits.hits._id,hits.hits._score,hits.hits._source.title,hits.hits._source.version")
...
puts "*** Profile suggestions #{response.code} #{response.message} in "+(elastic_out['took']/1000.0).to_s.yellow+" seconds"
