	...
	for i, node := range nodes {
		...
		if err != nil {
			...
			messages[i] = &externalResp.Node{
				...
				KernelRelease:         node.KernelRelease,
				KernelVersion:         node.KernelVersion,
				VirtualizationSystem:  node.VirtualizationSystem,
				VirtualizationRole:    node.VirtualizationRole,
				DmiSystemManufacturer: node.DmiSystemManufacturer,
				DmiSystemSerialNumber: node.DmiSystemSerialNumber,
				Domain:                node.Domain,
				Hostname:              node.Hostname,
				Macaddress:            node.Macaddress,
				MemoryTotal:           node.MemoryTotal,
				Ip6Address:            toString(node.Ip6address),
				Ipaddress:             toString(node.Ipaddress),
			}
		}
	}
	return messages
